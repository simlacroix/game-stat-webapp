...
import {
    GetEmblemImage,
    LoLLeagueEntry,
    LoLQueueTypeDescription,
    LolRankDescription,
    LoLTierDescription,
} from '../../../types/LeagueOfLegendsTypes';

export interface LolRankInfoProps {
    leagueEntry: LoLLeagueEntry;
}

const LolRankInfo = ({leagueEntry}: LolRankInfoProps) => {

    return (
        <PlayerStatContentContainer>
            <div className={'flex flex-col justify-between items-center lg:items-start'}>
                <span
                    className={'font-bold lg:font-extrabold text-white'}>Ranked {LoLQueueTypeDescription(leagueEntry.queueType)}</span>
